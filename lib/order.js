...
module.exports = function order (options) {
  ...
  function findAll (msg, done) {
    return order.list$({}, (err, orders) => {
      if (err) return done(err);
      if (!orders) return done(null, {msg: 'no record'});
  ...
  function findById (args, done) {
    return order.load$({id: args.id}, (err, order) => {
      if (err) return done(err);
      if (!order) return done(null, {msg: 'not found'});
      return done(null, {order: seneca.util.clean(order)});
    });
  }
